...
void MyWindow::processFiltration(MeasurementsController& rawMeasurement, const uint32_t deltaTimeMs)
{
    ...
    else
    {
        ...
        //filteredPositionY = positionUpdater.getTotalDistance();


        const double filteredXacc = 0.0F;// = kalmanFilters.getFilterForPosition().vecX()(0); //xAcc
        const double filteredYacc = 0.0F;// = kalmanFilters.getFilterForPosition().vecX()(3); //yAcc
...
void MyWindow::OnFilterFileGpsTimer(wxTimerEvent& event)
{
    ...
    const double timeMs, const uint32_t deltaTime)
{
    ...
    dataset->GetSerie(1)->SetName("filtered Y acceleration");
...
void MyWindow::prepareGui()
{
    ...
    filterFileGpsTimer.Bind(wxEVT_TIMER, &MyWindow::OnFilterFileGpsTimer, this);
